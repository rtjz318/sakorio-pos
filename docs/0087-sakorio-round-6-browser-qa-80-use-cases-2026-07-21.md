...
## Full end-to-end journey addendum - 50 complete user experience workflows
...
Use these IDs in the result document when this batch is executed: `R6-FLOW-001` to `R6-FLOW-050`.
...
| R6-FLOW-021 | P0 | Host, customer, waiter, cashier | Two separate reservations arrive at the same time -> host seats first table -> second reservation waits briefly -> waiter opens QR for first table -> host seats second table -> both tables order in parallel -> kitchen handles both -> cashier settles first, then second -> both tables close. | Concurrent arrivals do not confuse reservation state, QR tokens, KDS tickets, or table close flow. |
| R6-FLOW-022 | P0 | Host, customer, waiter | Customer reservation arrives with more guests than booked -> host updates/handles party size -> seats at a larger table if available -> customer QR orders -> kitchen serves -> cashier pays -> table resets. | Real-world party-size change is handled safely without corrupting reservation/table capacity. |
| R6-FLOW-023 | P0 | Customer, host, cashier | Customer reservation is seated -> customer scans QR -> customer accidentally opens the same QR on two phones -> both phones add items/submit in sequence -> cashier reviews one combined bill -> payment -> close. | Same-table multi-device QR usage does not duplicate, split, or leak sessions. |
| R6-FLOW-024 | P0 | Customer, waiter, kitchen, cashier | Customer QR order is placed -> waiter notices customer wants no spice/extra note -> waiter adds another item with note from POS -> KDS shows notes clearly -> kitchen serves -> payment -> close. | Notes/special requests remain clear across QR, POS, KDS, bill, and history. |
| R6-FLOW-025 | P0 | Waiter, kitchen, cashier | Waiter takes entire order in POS for elderly customer who does not use QR -> kitchen serves -> customer adds dessert verbally -> waiter adds dessert -> payment at cashier -> close table. | Pure staff-service flow is as smooth as QR flow and supports add-on rounds. |
| R6-FLOW-026 | P0 | Customer, waiter, cashier | Customer QR order is active -> customer asks waiter to add one more item verbally -> waiter adds staff POS item -> customer checks bill on QR -> cashier confirms same total -> payment -> table reset. | Customer QR bill and staff POS bill remain synchronized after mixed entry points. |
| R6-FLOW-027 | P0 | Customer, cashier | Customer orders through QR -> wants to pay at cashier with terminal instead of HitPay -> cashier finds table/order -> terminal payment -> customer QR reload shows paid/closed after table close. | Card-at-counter / terminal settlement works cleanly for QR-originated orders. |
| R6-FLOW-028 | P0 | Customer, cashier | Customer orders through QR -> starts HitPay but internet/browser freezes -> customer returns to cashier -> cashier checks unpaid state -> settles terminal -> closes table. | Payment uncertainty can be recovered without false paid/unpaid confusion. |
| R6-FLOW-029 | P0 | Cashier, manager | Cashier accidentally selects wrong table before adding items -> catches mistake before checkout -> clears/switches table -> enters order on correct table -> kitchen/payment/close. | Wrong-table prevention/recovery is obvious before an order is committed. |
| R6-FLOW-030 | P0 | Cashier, kitchen, manager | Cashier accidentally submits wrong item to kitchen -> kitchen has not started -> manager/cashier voids or corrects -> correct item is sent -> payment and close. | Pre-production mistake correction is safe and auditable. |
| R6-FLOW-031 | P1 | Cashier, kitchen, manager | Wrong item already started in kitchen -> cashier attempts correction -> system requires manager policy or documents blocked path -> corrected final bill is settled -> close. | After-production mistake policy is visible and cannot silently desync KDS/bill. |
| R6-FLOW-032 | P1 | Customer, cashier, manager | Customer complains after payment and needs partial refund/adjustment -> manager opens paid bill/history -> refund/adjust path is visible or safely blocked -> report reflects outcome. | Post-payment correction/refund policy is discoverable and audit-safe. |
| R6-FLOW-033 | P1 | Host, waiter, customer | Customer seated at table then requests to move due to aircon/noise after first order is placed -> staff moves table or blocks safely -> QR/session/KDS labels remain accurate -> payment/close. | Table move after order does not leave wrong table labels or stale QR access. |
| R6-FLOW-034 | P1 | Host, waiter, customer | Two tables request to merge into one larger party before ordering -> host/waiter handles move/merge or documents unsupported path -> one final bill or clear separate bills -> close. | Table merge scenario is either supported or safely unsupported with no orphan sessions. |
| R6-FLOW-035 | P1 | Host, waiter, customer | Large party wants separate bills after ordering together -> cashier explores split/partial payment support -> settles using supported workaround if needed -> closes table. | Split-bill limitation/support is clear before launch. |
| R6-FLOW-036 | P1 | Customer, cashier | Customer orders, pays, then realizes an item was missed and wants new order at same table before leaving -> staff starts allowed new order or requires close/new session -> settles/cleans safely. | Paid-but-not-closed follow-up ordering does not merge confusingly with paid bill. |
| R6-FLOW-037 | P1 | Host, queue, customer | Queue guest joins, then leaves before seating -> host marks removed/cancelled -> later same phone joins again -> new queue lifecycle proceeds -> seat/order/pay/close. | Queue cancellation and rejoin do not create duplicate active records. |
| R6-FLOW-038 | P1 | Host, reservation, queue | Fully booked period: reservation arrives late while queue walk-in is waiting -> host reviews states -> seats correct priority -> both workflows stay accurate. | Host can make real seating decisions without stale/confusing boards. |
| R6-FLOW-039 | P1 | Customer, host, cashier | Reservation customer changes date/time or cancels from public flow if available -> staff sees updated/cancelled state -> no table session created -> later walk-in uses table normally. | Public reservation changes/cancellations do not create operational residue. |
| R6-FLOW-040 | P1 | Customer, waiter, kitchen | Customer QR order includes unavailable/sold-out item if configured -> system blocks or flags before kitchen -> customer chooses replacement -> order served -> payment/close. | Unavailable item handling prevents silent bad tickets. |
| R6-FLOW-041 | P1 | Customer, waiter, kitchen | Customer submits order with item notes/modifiers -> kitchen marks started -> customer orders second round with different notes -> cashier reviews notes/history -> payment/close. | Notes/modifiers survive multiple rounds and remain readable. |
| R6-FLOW-042 | P1 | Kitchen, waiter, cashier | Kitchen has several active tickets -> one ticket is served late -> waiter checks Orders/POS state -> cashier cannot mistakenly close unpaid/unserved table without clear warning. | Late ticket operational state remains visible across KDS/POS/Orders. |
| R6-FLOW-043 | P1 | Kitchen, cashier, manager | KDS board is refreshed during rush -> tickets remain in correct lanes -> staff processes all -> Reports/Orders show paid history after settlement. | Refresh during rush does not drop or duplicate tickets. |
| R6-FLOW-044 | P1 | Tablet/iPad cashier, customer | On iPad landscape, cashier handles full table lifecycle: select table -> add items -> open QR -> view current orders -> payment -> close -> return to grid. | iPad cashier workflow is touch-friendly and does not require awkward scrolling/hit targets. |
| R6-FLOW-045 | P1 | Tablet/iPad host | On iPad portrait/landscape, host handles reservation arrival -> seat table -> queue walk-in -> close empty/no-order table. | Host iPad workflow keeps buttons visible and usable. |
| R6-FLOW-046 | P1 | Tablet/iPad kitchen | On iPad, kitchen processes mixed food/beverage tickets through all lanes during active service. | Kitchen board is readable and stable on tablet. |
| R6-FLOW-047 | P2 | Manager, staff, cashier | Manager creates a new staff user/role -> assigns or views shift -> staff clocks in -> staff performs a POS order -> clocks out -> manager checks attendance/report. | User, role, timetable, shift, POS, and attendance form a realistic staff lifecycle. |
| R6-FLOW-048 | P2 | Manager, staff | Staff applies/records annual leave or MC if available -> timetable reflects absence -> shift/attendance surface remains understandable -> manager reviews balance. | Leave/MC workflow is either launch-ready or clearly scoped as future work. |
| R6-FLOW-049 | P1 | Manager, cashier | Manager reviews Settings payment mode/status -> cashier performs terminal order and HitPay/customer order -> Reports reconcile both methods -> no secrets exposed in UI. | Payment configuration, payment execution, and reporting align safely. |
| R6-FLOW-050 | P0 | Owner, all roles | Full launch rehearsal: clean starting board -> create one reservation, one queue walk-in, one POS-only table, one QR-only table, one mixed POS+QR table -> process kitchen/beverage tickets -> settle payments -> close every table -> verify Orders history, Reports totals, Queue empty, Reservations statuses, Kitchen zero, QR tokens closed. | Restaurant can complete a realistic service cycle and return to a clean state ready for the next day. |
