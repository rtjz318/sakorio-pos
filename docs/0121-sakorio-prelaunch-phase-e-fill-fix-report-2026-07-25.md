...
| Fixed table QR activation | E2E-052 live retest showed fixed printed QR links can be visible/printable while the table session is still closed, with no obvious action in the Table QR panel to open ordering. | Added an explicit `Open QR ordering` action and open/closed status inside the Tables → Table QR drawer, then promoted the action above the fold in the table-service header so iPad/desktop staff do not need to scroll through the menu. | Staff can seat/open a table, then customers scanning the fixed QR can order without needing a regenerated QR. |
| Customer product add reachability | E2E-052 live customer QR retest showed the product detail `Add to cart` footer could render below the visible viewport and normal scrolling did not reach it. First retest on the 1280×720 live browser proved the button existed but the sheet stack was still too tall. | Added an immediate inline `Add to cart` action near the product title, made the detail footer sticky, and changed tablet/landscape product detail to a compact two-column sheet so the action stays above the fold. | Customers can add items from the QR menu reliably on shorter/tablet-height screens. |
